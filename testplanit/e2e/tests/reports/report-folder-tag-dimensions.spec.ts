...
test("test-execution report groups results by folder, flat and rolled up", async ({
  ...
  const ts = Date.now();
  const projectId = await api.createProject(`E2E FolderDim ${ts}`);
  const rootId = await api.getRootFolderId(projectId);
  const parentId = await api.createFolder(projectId, `Parent ${ts}`, rootId);
  const childId = await api.createFolder(projectId, `Child ${ts}`, parentId);
...
test("test-execution report groups results by tag with fan-out and a None group", async ({
  ...
  const ts = Date.now();
  const projectId = await api.createProject(`E2E TagDim ${ts}`);
  const rootId = await api.getRootFolderId(projectId);
  const taggedCase = await api.createTestCase(
    projectId,
    rootId,
    `Tagged ${ts}`
  );
  const untaggedCase = await api.createTestCase(
    projectId,
    rootId,
    `Untagged ${ts}`
  );

  const tagA = await api.createTag(`tag-a-${ts}`);
  const tagB = await api.createTag(`tag-b-${ts}`);
