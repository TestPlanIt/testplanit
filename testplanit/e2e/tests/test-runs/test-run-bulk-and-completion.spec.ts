...
test.describe("Test Run Bulk Operations and Completion", () => {
  test("should reflect bulk status updates for multiple cases in a test run", async ({
    ...
    let rc1Id: number | undefined;
    let rc2Id: number | undefined;
    let rc3Id: number | undefined;
    let passedStatusId: number | undefined;

    await test.step("Create project, folder, and three test cases", async () => {
      ...
      runId = await api.createTestRun(projectId, `Bulk Run ${ts}`);
      [rc1Id, rc2Id, rc3Id] = await api.addTestCasesToTestRun(runId, [
        case1Id,
        case2Id,
        case3Id,
      ]);
    ...
    await test.step("Apply the passed status to all three run cases", async () => {
      // Get the "passed" status ID for the project
      passedStatusId = await api.getStatusId("passed");
    ...
    await test.step("Verify all three cases report the passed status", async () => {
    ...
    await test.step("Cross-verify completion via the summary endpoint", async () => {
  ...
  test("should allow assigning a case to a user and verify assignment via API", async ({
    ...
    await test.step("Create project, case, run, and add the case to the run", async () => {
      ...
      runId = await api.createTestRun(projectId, `Assign Run ${ts}`);
      [testRunCaseId] = await api.addTestCasesToTestRun(runId, [caseId]);
    });

    await test.step("Assign the run case to the admin user", async () => {
      await api.assignTestRunCase(testRunCaseId!, adminUserId);
    });

    await test.step("Verify the assignment via API", async () => {
      const testRunCases = await api.getTestRunCases(runId!);
  ...
  test("should complete a test run via CompleteTestRunDialog on the detail page", async ({
    ...
    await test.step("Create project, case, and run", async () => {
      ...
      runId = await api.createTestRun(projectId, `Complete Run ${ts}`);
    ...
    const earlyReturn = await test.step(
      ...
      async () => {
        ...
        isCompleteButtonVisible = await completeButton
    ...
    await test.step("Open the dialog and confirm completion", async () => {
    ...
    await test.step("Verify the run is marked completed via API", async () => {
      const testRunResponse = await request.get(
        `/api/model/testRuns/findFirst?q=${encodeURIComponent(
          JSON.stringify({
            where: { id: runId },
  ...
  test("should complete a test run via the complete trigger on the runs list page", async ({
    ...
    await test.step("Create project and run", async () => {
      projectId = await api.createProject(`E2E CompleteList ${ts}`);
      runId = await api.createTestRun(projectId, `List Complete Run ${ts}`);
    });

    await test.step("Open the runs list and reveal the run's actions menu", async () => {
    ...
    await test.step("Trigger completion and confirm in the dialog", async () => {
    ...
    await test.step("Verify the run is completed via API", async () => {
      const testRunResponse = await request.get(
        `/api/model/testRuns/findFirst?q=${encodeURIComponent(
          JSON.stringify({
            where: { id: runId },
  ...
  test("should navigate between sibling config runs via config combobox", async ({
    ...
    let configSelector: ReturnType<typeof page.locator> | undefined;

    await test.step("Create project, two configurations, and two sibling runs", async () => {
      // Setup: create project with two configurations and two sibling test runs
      projectId = await api.createProject(`E2E NavConfig ${ts}`);
      ...
      run1Id = await api.createTestRun(projectId, `Nav Chrome Run ${ts}`, {
        configId: config1Id,
        configurationGroupId: groupId,
      });
      run2Id = await api.createTestRun(projectId, `Nav Firefox Run ${ts}`, {
        configId: config2Id,
        configurationGroupId: groupId,
      });
    ...
    await test.step("Open run1 and locate the config combobox", async () => {
    ...
    await test.step("Select the second config and confirm URL updates", async () => {
    ...
    await test.step("Deselect the first config and confirm navigation to run2", async () => {
  ...
  test("should show correct case count when switching between sibling config runs", async ({
    ...
    await test.step("Create project, two configs, and two sibling runs with different case counts", async () => {
      // Setup: create project with two configurations and two sibling runs with different case counts
      projectId = await api.createProject(`E2E CaseCount ${ts}`);
      ...
      const groupId = randomUUID();
      run1Id = await api.createTestRun(
        projectId,
      ...
      run2Id = await api.createTestRun(projectId, `CaseCount Edge Run ${ts}`, {
        configId: config2Id,
        configurationGroupId: groupId,
      });
    ...
    await test.step("Open run1 and verify it shows two cases", async () => {
    ...
    await test.step("Open run2 and verify it shows one case", async () => {
