...
test.describe("Auto-Tag Flow", () => {
  test("should show auto-tag button when cases are selected and LLM integration exists", async ({
    ...
    await test.step("Seed project, LLM integration, folder, and cases", async () => {
      projectId = await api.createProject(`E2E AutoTag ${ts}`);
      const llmId = await api.createLlmIntegration(`E2E LLM AutoTag ${ts}`);
      await api.linkLlmToProject(projectId, llmId);
      folderId = await api.createFolder(projectId, `AutoTag Folder ${ts}`);
    ...
    await test.step("Open the folder and wait for case rows", async () => {
    ...
    await test.step("Select all cases and verify the auto-tag button appears", async () => {
  ...
  test("should open auto-tag dialog and show analysis progress with mocked API", async ({
    ...
    await test.step("Mock the auto-tag submit, status, and apply API routes", async () => {
    ...
    await test.step("Select cases and open the auto-tag dialog", async () => {
  ...
  test("should verify auto-tag mock routes intercept correctly", async ({
    ...
    await test.step("Seed project and LLM integration", async () => {
      projectId = await api.createProject(`E2E AutoTag Routes ${ts}`);
    ...
    await test.step("Navigate to the repository and verify it loads", async () => {
