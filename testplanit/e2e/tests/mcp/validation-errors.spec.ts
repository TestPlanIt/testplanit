...
test.describe("CASE-12 — validation error surfaces at the REST API layer", () => {
  ...
  test.describe("Group 1 — case create with missing required relation (CASE-12 missing template)", () => {
    test("CASE-12 — POST create without template: connect returns non-2xx (host FK enforcement)", async ({ request, baseURL }) => {
      ...
      const hasError =
        ...
        body.errors != null ||
        body.code != null;
      expect(hasError, `Expected error body, got: ${JSON.stringify(body)}`).toBe(true);
    });
  });
  ...
  test.describe("Group 2 — folder soft-delete enforcement (CASE-10 / CASE-12 MCP layer)", () => {
    ...
    test("setup — create a test folder for the enforcement check", async ({ request, baseURL }) => {
    ...
    test("setup — create a case inside the folder", async ({ request, baseURL }) => {
      const r = await request.post(`${baseURL}/api/model/repositoryCases/create`, {
        headers: ctx.headers,
        data: {
          data: {
    ...
    test("CASE-12 / CASE-10 — host REST API allows soft-delete of non-empty folder (enforcement is in MCP tool layer, not REST layer)", async ({ request, baseURL }) => {
    ...
    test("cleanup — soft-delete the case that was inside the folder", async ({ request, baseURL }) => {
