...
test.describe("Copy/Move dialog same-project", () => {
  ...
  test("setup: create project + folders + 3 cases", async ({ api }) => {
    ...
    await test.step("Create project, root folder, sibling and nested folders", async () => {
    ...
    await test.step("Create 3 source test cases in the root folder", async () => {
    ...
    await test.step("Verify all created ids are valid", async () => {
  ...
  test("Same-project copy via dialog: pick sibling folder, copy 1 case", async ({
    ...
    await test.step("Confirm current-project suffix and pick the sibling folder", async () => {
    ...
    await test.step("Confirm copy is default, choose rename, and run the copy", async () => {
    ...
    await test.step("Verify copy completed and close the dialog", async () => {
    ...
    await test.step("Verify a new case exists in the sibling folder and track it", async () => {
      ...
      const newCases = (
        body.data as Array<{ id: number; name: string }>
      ).filter((c) => c.id !== sourceCaseAId);
  ...
  test("Same-project move via dialog: pick nested folder, move 1 case", async ({
    ...
    await test.step("Verify move completed and close the dialog", async () => {
      await expect(page.getByTestId("copy-move-close-button")).toBeVisible({
        timeout: 30_000,
      });
  ...
  test("Multi-select copy via dialog: 2 cases to sibling folder uses plural ICU branch", async ({
    ...
    await test.step("Choose rename and run the multi-select copy", async () => {
    ...
    await test.step("Verify plural copy summary and close the dialog", async () => {
      await expect(page.getByTestId("copy-move-close-button")).toBeVisible({
        timeout: 30_000,
      });
    ...
    await test.step("Verify exactly 2 new cases added to the sibling folder and track them", async () => {
