...
test.describe("Steps Display", () => {
  ...
  test.beforeAll(async ({ api }) => {
    ...
    stepsDisplayProjectId = await api.createProject(
      `E2E Steps Display Project ${uniqueId}`
    );
    ...
    try {
      ...
      if (handleBox) {
        ...
        if (handleBox.x < 100 || handleBox.x < targetX - 50) {
          console.log(
            `Expanding left panel by dragging handle from ${handleBox.x}px to ${targetX}px`
          );
          ...
          console.log(
            `Left panel is already wide enough (handle at ${handleBox.x}px)`
          );
        }
      }
    } catch (error) {
      console.log(
        "Could not expand panel by dragging - this may indicate a problem with the page load"
      );
  ...
  test("Read-Only Steps Display on Test Case Details Page", async ({
    ...
    await test.step("Create test case with three steps via API", async () => {
      const folderName = `Steps Display Folder ${uniqueId}`;
      const folderId = await api.createFolder(projectId, folderName);
      testCaseName = `Steps Display Case ${uniqueId}`;
            ...
            content: [
              {
                type: "paragraph",
                content: [
                  { type: "text", text: "Step 1: Open the application" },
                ],
              },
            ],
          },
            ...
            content: [
              {
                type: "paragraph",
                content: [
                  { type: "text", text: "Step 2: Login with credentials" },
                ],
              },
            ],
          },
        ...
        },
        {
            ...
            content: [
              {
                type: "paragraph",
                content: [
                  { type: "text", text: "Step 3: Navigate to dashboard" },
                ],
              },
            ],
          },
          expectedResult: {
            ...
            content: [
              {
                type: "paragraph",
                content: [{ type: "text", text: "Dashboard is displayed" }],
              },
            ],
          },
          order: 2,
    ...
    await test.step("Open test case details page and confirm correct case", async () => {
    ...
    await test.step("Verify all step content is displayed", async () => {
      // Verify step content is displayed
      await expect(
        page.locator("text=Step 1: Open the application")
      ).toBeVisible({ timeout: 15000 });
      await expect(
        page.locator("text=Application opens successfully")
      ).toBeVisible();
      await expect(
        page.locator("text=Step 2: Login with credentials")
      ).toBeVisible();
      await expect(page.locator("text=User is logged in")).toBeVisible();
      await expect(
        page.locator("text=Step 3: Navigate to dashboard")
      ).toBeVisible();
  ...
  test("Steps Edit Mode with Add/Remove", async ({ api, page }) => {
    ...
    await test.step("Create test case with one initial step via API", async () => {
      const folderName = `Steps Edit Folder ${uniqueId}`;
      const folderId = await api.createFolder(projectId, folderName);
      testCaseName = `Steps Edit Case ${uniqueId}`;
      testCaseId = await api.createTestCase(
        projectId,
        folderId,
        testCaseName,
      ...
      await api.addStepsToTestCase(testCaseId, [
        {
          step: {
            ...
            content: [
              {
                type: "paragraph",
                content: [{ type: "text", text: "Initial step" }],
              },
            ],
          },
          expectedResult: {
    ...
    await test.step("Open test case details page", async () => {
      // Navigate to test case details page
      await page.goto(`/en-US/projects/repository/${projectId}/${testCaseId}`);
    ...
    await test.step("Enter edit mode and confirm existing step", async () => {
    ...
    await test.step("Add a step and verify two step editors with delete buttons", async () => {
      ...
      const addStepButton = page
        ...
        .filter({ hasNotText: "Add Link" })
        .filter({ hasNotText: "Add to Test Run" })
        .first();
  ...
  test("Steps Display in Test Run Execution View", async ({ api, page }) => {
    ...
    await test.step("Create test case with two steps via API", async () => {
      const folderName = `Run Steps Folder ${uniqueId}`;
      const folderId = await api.createFolder(projectId, folderName);
      testCaseName = `Run Steps Case ${uniqueId}`;
      testCaseId = await api.createTestCase(
        projectId,
        folderId,
        testCaseName,
      ...
      await api.addStepsToTestCase(testCaseId, [
        {
          step: {
            ...
            content: [
              {
                type: "paragraph",
                content: [{ type: "text", text: "Test run step 1" }],
              },
            ],
          },
          expectedResult: {
            ...
            content: [
              {
                type: "paragraph",
                content: [{ type: "text", text: "Expected result 1" }],
              },
            ],
          },
          order: 0,
        },
        {
          step: {
            ...
            content: [
              {
                type: "paragraph",
                content: [{ type: "text", text: "Test run step 2" }],
              },
            ],
          },
          expectedResult: {
            ...
            content: [
              {
                type: "paragraph",
                content: [{ type: "text", text: "Expected result 2" }],
              },
            ],
          },
          order: 1,
    ...
    await test.step("Create test run and add the test case via API", async () => {
    ...
    await test.step("Open the test case sidebar from the run table", async () => {
    ...
    await test.step("Verify steps are displayed in the sidebar", async () => {
      // Wait for steps to be visible in sidebar
      await expect(page.locator("text=Test run step 1")).toBeVisible({
        timeout: 15000,
      });
  ...
  test("Steps Diff Display in Version Comparison", async ({ api, page }) => {
    ...
    await test.step("Create test case and open it in edit mode", async () => {
      ...
      const folderId = await api.createFolder(projectId, folderName);
      testCaseName = `Diff Steps Case ${uniqueId}`;
      testCaseId = await api.createTestCase(
        projectId,
        folderId,
        testCaseName,
    ...
    await test.step("Add four steps to create version 2", async () => {
      // Add step 1
      await addStepButton.click();
      await page.waitForTimeout(500);
    ...
    await test.step("Save version 2 and verify its content", async () => {
      ...
      await expect(page.locator("text=Step 1 original")).toBeVisible({
        timeout: 10000,
      });
    ...
    await test.step("Reopen the test case in edit mode for version 3", async () => {
    ...
    await test.step("Edit, delete, and add steps to build version 3", async () => {
      // 1. Edit step 1 text
      ...
      const step2ExpectedResult = page
        .locator('[data-testid="step-editor-1"]')
        .locator(".tiptap")
        .nth(1);
      ...
      const deleteStep4Button = page
        .locator('[data-testid="delete-step-3"]')
        .first();
      ...
      await addStepButton.click();
      await page.waitForTimeout(500);
      const newStep1Editor = page
        .locator('[data-testid="step-editor-3"]')
        .first();
      ...
      await addStepButton.click();
      await page.waitForTimeout(500);
      const newStep2Editor = page
        .locator('[data-testid="step-editor-4"]')
        .first();
    ...
    await test.step("Save version 3 and verify its content", async () => {
      ...
      versionDropdown = page.locator('[data-testid="version-select-trigger"]');
      ...
      await expect(page.locator("text=Step 1 EDITED")).toBeVisible({
        timeout: 10000,
      });
    ...
    await test.step("Switch to version 2 and verify diff content", async () => {
  ...
  test("Steps Display in Add Result Modal", async ({ api, page }) => {
    ...
    await test.step("Create test case with two steps via API", async () => {
      ...
      const folderId = await api.createFolder(projectId, folderName);
      testCaseName = `Result Modal Case ${uniqueId}`;
      const templateId = getTestTemplateId();
      testCaseId = await api.createTestCase(
        projectId,
        folderId,
      ...
      await api.addStepsToTestCase(testCaseId, [
        {
          step: {
            ...
            content: [
              {
                type: "paragraph",
                content: [{ type: "text", text: "Result step 1" }],
              },
            ],
          },
          expectedResult: {
            ...
            content: [
              {
                type: "paragraph",
                content: [{ type: "text", text: "Result expected 1" }],
              },
            ],
          },
          order: 0,
        },
        {
          step: {
            ...
            content: [
              {
                type: "paragraph",
                content: [{ type: "text", text: "Result step 2" }],
              },
            ],
          },
          expectedResult: {
            ...
            content: [
              {
                type: "paragraph",
                content: [{ type: "text", text: "Result expected 2" }],
              },
            ],
          },
          order: 1,
    ...
    await test.step("Open the test case sidebar and the Add Result modal", async () => {
      // Click on the test case name in the table to open sidebar
      const testCaseLink = page.locator(`text=${testCaseName}`).first();
      await expect(testCaseLink).toBeVisible({ timeout: 10000 });
      await testCaseLink.click();
    ...
    await test.step("Verify steps are displayed in the Add Result modal", async () => {
  ...
  test("Shared Step Group Display", async ({ api, page }) => {
    ...
    await test.step("Create a shared step group via API", async () => {
        ...
        [
          {
            step: {
              ...
              content: [
                {
                  type: "paragraph",
                  content: [{ type: "text", text: "Shared step 1" }],
                },
              ],
            },
            expectedResult: {
              ...
              content: [
                {
                  type: "paragraph",
                  content: [{ type: "text", text: "Shared result 1" }],
                },
              ],
            },
            order: 0,
          },
          {
            step: {
              ...
              content: [
                {
                  type: "paragraph",
                  content: [{ type: "text", text: "Shared step 2" }],
                },
              ],
            },
            expectedResult: {
              ...
              content: [
                {
                  type: "paragraph",
                  content: [{ type: "text", text: "Shared result 2" }],
                },
              ],
            },
            order: 1,
      ...
      const folderId = await api.createFolder(projectId, folderName);
      testCaseName = `Shared Steps Case ${uniqueId}`;
      const templateId = getTestTemplateId();
      testCaseId = await api.createTestCase(
        projectId,
        folderId,
      ...
      await api.addStepsToTestCase(testCaseId, [
        {
          step: {
            ...
            content: [
              {
                type: "paragraph",
                content: [{ type: "text", text: "Regular step" }],
              },
            ],
          },
          expectedResult: {
    ...
    await test.step("Open the test case details page", async () => {
      // Navigate to test case details page
      await page.goto(`/en-US/projects/repository/${projectId}/${testCaseId}`);
      await page.waitForLoadState("networkidle");
    ...
    await test.step("Verify regular step and shared step group are displayed", async () => {
  ...
  test("Steps Display Preserves Order", async ({ api, page }) => {
    ...
    await test.step("Create test case with three ordered steps via API", async () => {
    ...
    await test.step("Verify steps appear in order with visible badges", async () => {
