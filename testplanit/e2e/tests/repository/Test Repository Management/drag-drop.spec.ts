...
test.describe("Drag & Drop", () => {
  ...
  test("Drag Folder to New Position (Same Level)", async ({ api, page }) => {
    ...
    await test.step("Create project and two root folders", async () => {
    ...
    let folder1: ReturnType<RepositoryPage["getFolderById"]> | undefined;
    let folder2: ReturnType<RepositoryPage["getFolderById"]> | undefined;
    let folder1YBefore: number | undefined;
    await test.step("Open repository and record initial folder positions", async () => {
      await repositoryPage.goto(projectId!);

      // Wait for both folders to be visible
      folder1 = repositoryPage.getFolderById(folder1Id!);
      folder2 = repositoryPage.getFolderById(folder2Id!);
    ...
    await test.step("Verify folder order change after drag", async () => {
  ...
  test("Drag Folder to Become Subfolder", async ({ api, page }) => {
    ...
    await test.step("Create project with parent and child folders", async () => {
      projectId = await getTestProjectId(api);

      const parentName = `ParentDrop ${uniqueId}`;
      parentId = await api.createFolder(projectId, parentName);
      childName = `ChildDrop ${uniqueId}`;
      childId = await api.createFolder(projectId, childName);
    });
    ...
    await test.step("Open repository and drag child onto parent to nest it", async () => {
      await repositoryPage.goto(projectId!);
    ...
    await test.step("Expand parent folder if it gained children", async () => {
    ...
    await test.step("Verify child remains visible and check indentation", async () => {
  ...
  test("Drag Subfolder to Root Level", async ({ api, page }) => {
    ...
    let parentId: number | undefined;
    let childId: number | undefined;
    await test.step("Create project with parent and nested child folders", async () => {
      projectId = await getTestProjectId(api);
    ...
    let child: ReturnType<RepositoryPage["getFolderById"]> | undefined;
    let childXBefore: number | undefined;
    await test.step("Open repository, expand parent, and record nested child position", async () => {
      await repositoryPage.goto(projectId!);
      ...
      child = repositoryPage.getFolderById(childId!);
    ...
    await test.step("Verify child indentation after drag to root", async () => {
  ...
  test("Drag Test Case to Different Folder", async ({ api, page }) => {
    ...
    await test.step("Create project, source and target folders, and a test case", async () => {
      projectId = await getTestProjectId(api);

      const sourceFolder = `Source Drag ${Date.now()}`;
      sourceFolderId = await api.createFolder(projectId, sourceFolder);
      const targetFolder = `Target Drag ${Date.now()}`;
      targetFolderId = await api.createFolder(projectId, targetFolder);
      caseName = `Draggable Case ${Date.now()}`;
      await api.createTestCase(projectId, sourceFolderId, caseName);
    });

    await test.step("Open source folder and drag the test case to the target folder", async () => {
      await repositoryPage.goto(projectId!);
    ...
    await test.step("Verify the case moved to target or remained in source", async () => {
  ...
  test("Drag Multiple Test Cases to Folder", async ({ api, page }) => {
    ...
    await test.step("Create project, source and target folders, and two test cases", async () => {
      projectId = await getTestProjectId(api);
    ...
    let case1IdValue: number | undefined;
    await test.step("Open source folder and select both test cases", async () => {
      await repositoryPage.goto(projectId!);
      ...
      case1IdValue = case1Id;
    });

    await test.step("Drag the selected cases to the target folder", async () => {
      // Drag selection to target folder
      const selectedRow = page
        .locator(`[data-testid="case-row-${case1IdValue!}"]`)
        .first();
      const targetFolderElement = repositoryPage.getFolderById(targetFolderId!);
  ...
  test("Drag and Drop Visual Feedback - Valid Target", async ({
    ...
    let sourceFolderId: number | undefined;
    let targetFolderId: number | undefined;
    await test.step("Create project with source and target folders", async () => {
      projectId = await getTestProjectId(api);
    ...
    let sourceFolder: ReturnType<RepositoryPage["getFolderById"]> | undefined;
    let targetFolder: ReturnType<RepositoryPage["getFolderById"]> | undefined;
    await test.step("Drag source folder toward target and check drop highlight", async () => {
      await repositoryPage.goto(projectId!);

      sourceFolder = repositoryPage.getFolderById(sourceFolderId!);
      targetFolder = repositoryPage.getFolderById(targetFolderId!);
      ...
      const hasDropIndicator = await dropIndicator
        .isVisible()
        .catch(() => false);
    ...
    await test.step("Verify both folders remain visible after drag", async () => {
  ...
  test("Drag and Drop Visual Feedback - Invalid Target", async ({
    ...
    await test.step("Create project with a folder and a test case", async () => {
      projectId = await getTestProjectId(api);
    ...
    await test.step("Drag the case onto its own folder and verify no valid drop target", async () => {
      await repositoryPage.goto(projectId!);
  ...
  test("Cancel Drag Operation", async ({ api, page }) => {
    let projectId: number | undefined;
    let folderId: number | undefined;
    await test.step("Create project with a single folder", async () => {
      projectId = await getTestProjectId(api);
    ...
    await test.step("Start dragging the folder, then cancel with Escape", async () => {
      await repositoryPage.goto(projectId!);
    ...
    await test.step("Verify folder remains in its original position", async () => {
  ...
  test("Drag Folder to Bottom of Root Level", async ({ api, page }) => {
    let projectId: number | undefined;
    let folder3Id: number | undefined;
    await test.step("Create project with three root folders", async () => {
      projectId = await getTestProjectId(api);
      ...
      folder3Id = await api.createFolder(projectId, folder3Name);
    });

    await test.step("Drag the third folder to the bottom of the tree", async () => {
      await repositoryPage.goto(projectId!);
  ...
  test("Hierarchical Folder Move Validation", async ({ api, page }) => {
    ...
    let parent: ReturnType<RepositoryPage["getFolderById"]> | undefined;
    await test.step("Open repository, expand parent, and drag parent into its child", async () => {
      await repositoryPage.goto(projectId!);
      ...
      parent = repositoryPage.getFolderById(parentId!);
    ...
    await test.step("Verify the invalid move was prevented and parent stays at root", async () => {
