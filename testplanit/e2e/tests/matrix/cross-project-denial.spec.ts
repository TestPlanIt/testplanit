...
test.describe("Matrix view — cross-project denial @matrix", () => {
  test("non-admin user is denied on all four matrix endpoints", async ({
    ...
    const targetProjectId = await api.createProject(
    ...
    const noAccessUserId = userResult.data.id;
    ...
    try {
      ...
      expect(exportRes.status(), "export GET must reject non-member").toBe(403);
