...
test.describe("Webhook outbound config delete — card disappears + no future fan-out (M-04)", () => {
  ...
  test("admin clicks Delete, confirms AlertDialog, card disappears, no future events fan out to the deleted destination", async ({
    ...
    await test.step("Open the outbound webhooks tab and confirm the target card is rendered", async () => {
      await page.goto(
    ...
    await test.step("Cancel the delete dialog then re-open and confirm the deletion", async () => {
      await page
    ...
    await test.step("Verify the success toast renders and the card is removed", async () => {
    ...
    //    of the config.
    await test.step("Fire a new test_run.completed event and assert no delivery lands on the deleted config", async () => {
