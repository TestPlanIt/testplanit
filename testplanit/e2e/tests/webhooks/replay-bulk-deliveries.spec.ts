...
test.describe("Webhook delivery bulk replay — outbound-only count", () => {
  ...
  test("admin sees 'Bulk replay 5 outbound failures' label and bulk replay enqueues only the 5 outbound rows; 2 inbound originals have zero replays", async ({
    ...
    await page.getByTestId("webhook-outbound-name-input").fill("E2E Bulk Stub");
    ...
    const inboundSecret = await githubCard
    ...
    // 3. Seed 5 failed OUTBOUND rows via direct Prisma writes. Each has a
    //    paired WebhookOutboxEvent so the replay service can find it.
    ...
    // 4. Seed 2 failed INBOUND rows via the no_handler flow on the GitHub
    //    config. These end up on inboundConfigId, NOT outboundConfigId, so
    //    bulk replay (scoped to outboundConfigId) must not touch them.
    ...
    // 5. Navigate to the Deliveries tab with the outbound config selected
    //    + status=failed + since-yesterday. Bulk-replay button gates on
    //    configIds.length===1 AND status===failed AND outboundFailedCount>0.
    ...
    // 6. Assert bulk-replay button is visible AND label includes the OUTBOUND
    //    count (5), NOT the total failure count (7). The wording "outbound" in
    //    the label is the cross-direction contract — inbound failures (which
    //    are scoped out by configId here AND by direction in the server
    //    action) never enter this count.
    ...
    // 7. Click bulk replay → AlertDialog confirms count = 5 (NOT 7).
    ...
    // 8. Toast confirms the batch was started.
    await expect(page.getByText(/Bulk replay started/i)).toBeVisible({
      timeout: 10_000,
    });

    // 9. Wait for all 5 replay rows to land. Each replay enqueues a dispatch
    //    job that POSTs to the (200) stub → success row written.
    ...
    for (const row of replayRows) {
      ...
      expect(seededOutboundDeliveryIds).toContain(row.replayedFromDeliveryId!);
      expect(row.webhookConfigId).toBe(outboundConfigId);
      expect(row.error).toBeNull(); // 200 stub
      expect(row.statusCode).toBe(200);
    }

    // 10. Defense-in-depth: the 2 inbound originals have ZERO replay rows.
    //     bulk replay filters by direction=OUTBOUND in its SELECT (Plan 04-06)
    //     and is scoped to outboundConfigId by webhookConfigId — inbound rows
    //     in the same project/timeframe must never be enqueued.
