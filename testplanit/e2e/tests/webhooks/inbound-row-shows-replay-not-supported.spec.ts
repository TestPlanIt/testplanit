...
test.describe("Webhook deliveries — inbound row shows replay-not-supported banner", () => {
  ...
  test("admin clicks failed inbound row, drawer opens with payloadDigest + banner + no Replay button", async ({
    ...
    await test.step("Configure a GitHub inbound webhook and capture its token and secret", async () => {
    ...
    await test.step("Seed a failed inbound delivery via the no_handler flow", async () => {
    ...
    await test.step("Open the Deliveries tab and view details of the failed row", async () => {
      await page.goto(
    ...
    await test.step("Verify the drawer shows payloadDigest, the banner, and no Replay button", async () => {
      ...
      await expect(drawer.getByTestId("webhook-drawer-event-id")).toHaveCount(
        0
      );

      // 4c. Gray "cannot be replayed" banner is rendered.
      const banner = drawer.getByTestId(
        "webhook-delivery-replay-not-supported"
      );
