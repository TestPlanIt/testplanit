...
test.describe("Shared datasets - owner + shared coexistence (Amendment A) @shared-datasets", () => {
  test("assign a shared dataset to a case with a local dataset; both coexist", async ({
    ...
    const projectId = await api.createProject(`E2E Owner+Shared ${Date.now()}`);
    ...
    // ---------- Setup: declare params + add local rows ----------
    ...
    // ---------- Setup: create a shared dataset in the SAME project ----------
    ...
    // ---------- Switch to Shared with an owner dataset already present ----------
    ...
    // ---------- Coexistence visible in both views ----------
