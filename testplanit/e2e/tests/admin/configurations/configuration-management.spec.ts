...
test.describe("Configuration Management - Page Display", () => {
  test("Admin can view configurations page", async ({ page }) => {
    await test.step("Open the configurations page", async () => {
      await page.goto("/en-US/admin/configurations");
      await page.waitForLoadState("networkidle");
    });

    await test.step("Verify page header, Categories section, and Add category button", async () => {
...
test.describe("Configuration Management - Category CRUD", () => {
  test("Admin can create a configuration category", async ({
    ...
    try {
      ...
      await test.step("Open the Add category form", async () => {
      ...
      await test.step("Enter the category name and submit", async () => {
      ...
      await test.step("Verify the new category appears and capture its ID", async () => {
  ...
  test("Admin can edit a configuration category", async ({
    ...
    try {
      ...
      await test.step("Open the edit dialog for the category", async () => {
      ...
      await test.step("Verify the edit dialog shows the original name and close it", async () => {
      ...
      await test.step("Rename the category via API", async () => {
        // Update via API
        await request.put(`${baseURL}/api/model/configCategories/update`, {
          data: {
            where: { id: categoryId },
      ...
      await test.step("Reload and verify the updated name replaced the original", async () => {
  ...
  test("Admin can delete a configuration category", async ({
    ...
    await test.step("Click delete on the category row", async () => {
      // Find the row
      const categoryRow = getCategoryRow(page, categoryName);
      await expect(categoryRow).toBeVisible({ timeout: 10000 });
    ...
    await test.step("Verify the category row is gone", async () => {
...
test.describe("Configuration Management - Variant CRUD", () => {
  test("Admin can create a variant within a category", async ({
    ...
    try {
      ...
      await test.step("Expand the category and open the Add Variant form", async () => {
      ...
      await test.step("Enter the variant name and save", async () => {
  ...
  test("Admin can edit a variant", async ({ page, request, baseURL }) => {
    ...
    try {
      ...
      await test.step("Expand the category and open the variant edit dialog", async () => {
      ...
      await test.step("Rename the variant and submit", async () => {
      ...
      await test.step("Reload, re-expand the category, and verify the updated variant name", async () => {
  ...
  test("Admin can delete a variant", async ({ page, request, baseURL }) => {
    ...
    try {
      ...
      await test.step("Expand the category and click delete on the variant", async () => {
        // Expand category row
        const categoryRow = getCategoryRow(page, categoryName);
        await expect(categoryRow).toBeVisible({ timeout: 10000 });
      ...
      await test.step("Confirm the variant deletion", async () => {
  ...
  test("Admin can open the configuration wizard", async ({
    ...
    try {
      ...
      await test.step("Open the configuration wizard", async () => {
        ...
        variantDialog = page.getByRole("dialog");
        await expect(variantDialog).toBeVisible({ timeout: 5000 });
      });

      await test.step("Verify the created category is selectable and close the wizard", async () => {
