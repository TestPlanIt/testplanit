...
test.describe("Configuration Project Scoping - Admin", () => {
  test("Projects column lists the projects a configuration is assigned to", async ({
    ...
    let row: ReturnType<typeof page.getByTestId> | undefined;

    await test.step("Create a project and a configuration assigned to it", async () => {
      projectId = await api.createProject(projectName);
      configId = await api.createConfiguration(configName, projectId);
    });

    await test.step("Open the Admin Configurations page and filter to the configuration by name", async () => {
  ...
  test("project filter shows assigned configurations and hides unassigned ones", async ({
    ...
    let unassignedId: number | undefined;

    await test.step("Create a project, an assigned configuration, and a global unassigned configuration", async () => {
      const projectId = await api.createProject(projectName);

      assignedId = await api.createConfiguration(assignedName, projectId);
      // No projectId -> global, not assigned to the new project.
      unassignedId = await api.createConfiguration(unassignedName);
    });

    await test.step("Open the Admin Configurations page and filter to the project", async () => {
      await page.goto("/en-US/admin/configurations");
      await page.waitForLoadState("load");
