// Live-DB integration test inventory scaffold for the covering-cases
// drill-down's latest-result extension (COV-01/COV-02, criterion 5).
// Titles only — converted by 26-03. Guard copied verbatim from
...
describeIntegration(
  ...
  () => {
    ...
    it.todo("returns the latest result for each covering case against real Postgres");
    it.todo(
      "prefers the newest execution across runs and across manual and automated results"
    );
    it.todo("returns a null status for a covering case that has never been executed");
    it.todo("still agrees with the rollup linked-case count after the extension");
    it.todo("keeps a case linked at two levels de-duplicated to one row");
  }
);
