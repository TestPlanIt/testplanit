...
export function EditCaseFieldModal({ casefield }: EditCaseFieldModalProps) {
  ...
  const FormSchema = z
    .object({
      displayName: z.string().min(1, {
        message: tCommon("fields.options.validation.displayNameRequiredCase"),
      }),
    ...
    .refine(
      ...
      {
        message: tCommon("fields.options.validation.minMaxValueInteger"),
  ...
  const existingSystemNames = useMemo(() => {
    const names: string[] = [];
    const exclude = casefield.systemName
      ? casefield.systemName.trim().toLowerCase()
      : null;
    existingCaseFields?.forEach((field) => {
      if (field.id === casefield.id) {
        return;
      }
  ...
  const defaultFormValues = useMemo(() => {
    return {
      ...
      systemName: casefield.systemName,
      isEnabled: casefield.isEnabled,
      isRequired: casefield.isRequired,
      isRestricted: casefield.isRestricted,
      typeId: casefield.type?.id.toString(),
      hint: casefield.hint ? casefield.hint : "",
      defaultValue: casefield.defaultValue ?? undefined,
      isChecked: casefield.isChecked || false,
      minValue: casefield.minValue,
      maxValue: casefield.maxValue,
      fieldOptions: casefield.fieldOptions,
      initialHeight: casefield.initialHeight,
    };
  }, [
    casefield.displayName,
    casefield.systemName,
    casefield.isEnabled,
    casefield.isRequired,
    casefield.isRestricted,
    casefield.type?.id,
    casefield.hint,
  ...
  useEffect(() => {
    if (open && !dropdownOptionsInitialized && casefield?.fieldOptions) {
      const transformedOptions = casefield.fieldOptions
        .map(({ fieldOption }) => ({
          id: fieldOption.id,
  ...
  }, [open, casefield?.fieldOptions, dropdownOptionsInitialized]);
  ...
  useEffect(() => {
    if (open) {
      const foundType = types?.find((type) => type.id === casefield.type.id);
  ...
  const renderOptions = (options: any) => {
    ...
    const dynamicOptionHelpKeys: Record<string, string> = {
      defaultValue: "caseField.defaultValue",
      isChecked: "caseField.isChecked",
      minValue: "caseField.minValue",
      maxValue: "caseField.maxValue",
      minIntegerValue: "caseField.minValue", // Assuming min/max Integer share same help text as Number
      maxIntegerValue: "caseField.maxValue", // Assuming min/max Integer share same help text as Number
      initialHeight: "caseField.initialHeight",
      dropdownOptions: "caseField.dropdownOptions",
    };
  ...
  async function onSubmit(data: z.infer<typeof FormSchema>) {
    ...
    try {
      // Update the CaseField properties
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="flex items-center">
                    {tCommon("fields.hint")}
                    <HelpPopover helpKey="caseField.hint" />
                  </FormLabel>
                  <FormControl>
                    <Input {...field} />
                  </FormControl>
                ...
                <FormItem>
                  <FormLabel className="whitespace-nowrap items-center flex space-x-2">
                    <div className="flex items-center">
                      ...
                      <HelpPopover helpKey="caseField.fieldType" />
                    </div>
                    <div className="text-muted-foreground text-sm">
                  ...
                  <FormControl>
                    <Controller
                      ...
                      render={() => (
                        <Select disabled>
                          <SelectTrigger>
                            <SelectValue placeholder={casefield.type.type} />
                          </SelectTrigger>
                        </Select>
                      )}
                    />
