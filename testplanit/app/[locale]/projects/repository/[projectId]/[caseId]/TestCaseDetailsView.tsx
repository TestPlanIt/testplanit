...
interface TestCaseDetailsViewProps {
  /** Case id when rendered in the repository list Sheet; overrides the route param. */
  caseIdOverride?: string;
  /** Project id when rendered in the repository list Sheet; overrides the route param. */
  projectIdOverride?: string;
  /** True when rendered in the slide-over Sheet rather than the full-page route. */
  inSheet?: boolean;
...
export function TestCaseDetailsView({
  ...
  const setRootRef = useCallback((node: HTMLFormElement | null) => {
    headerResizeObserverRef.current?.disconnect();
    if (node && typeof ResizeObserver !== "undefined") {
      // Measure synchronously as the node attaches (commit phase, before paint)
      // so a remount — e.g. navigating to the next case — paints the collapsed
      // layout directly instead of flashing the expanded buttons before the
      // async ResizeObserver collapses them.
      setHeaderCompact(node.offsetWidth > 0 && node.offsetWidth < 768);
      const ro = new ResizeObserver((entries) => {
        const width = entries[0]?.contentRect.width ?? 0;
        setHeaderCompact(width > 0 && width < 768);
      });
      ro.observe(node);
  ...
  // When the pane is narrow, the edit-mode action buttons collapse to icon-only
  // and reveal their label on hover (mirrors the QuickScript/Edit buttons).
  const compactActionClass = headerCompact
    ? "group gap-0 hover:gap-2 transition-all duration-200"
    : "";
  ...
  if (!data || error) {
    if (inSheet) {
      // Inside the Sheet we don't own the route — close the Sheet instead of
      // navigating the list underneath it.
        ...
        <div>
          ...
          <CardHeader className="pt-2">
            <CardTitle>
              ...
              <div className="flex items-start justify-between text-primary text-lg md:text-xl max-w-full">
                    ...
                    <div className="space-y-2 w-full">
                              ...
                              <TooltipTrigger asChild>
                                <span tabIndex={0}>
                                  <Button
                                    ...
                                    variant="default"
                                    disabled
                                    className={`ring-2 ring-destructive ring-offset-2 ring-offset-background ${compactActionClass}`}
                                  >
                                    {renderActionButtonContent(
                                      Save,
                                      t("common.actions.save")
                      ...
                      {!isAutomatedCaseSource(testcase.source) && (
                        <div className="w-full flex justify-end">
                          <Button
                            variant="destructive"
                            className={`px-2 py-1 h-auto ${compactActionClass || "w-full"}`}
                            type="button"
                          ...
                          {isDeleteCaseOpen && (
                            <DeleteCaseModal
                              ...
                              onDeleteSuccess={inSheet ? onClose : undefined}
                            />
                          )}
                        </div>
                      )}
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuItem
                            ...
                            data-testid="case-actions-activity-log"
                          >
                            <History className="me-2 h-4 w-4" />
                            <span>{t("common.fields.activityLog")}</span>
                          </DropdownMenuItem>
                          {versions && versions.length > 1 && (
                            <DropdownMenuSub>
                              <DropdownMenuSubTrigger
                                className="flex items-center cursor-pointer"
                                data-testid="case-actions-version"
                              >
                                <GitBranch className="me-2 h-4 w-4" />
                                <span>{t("common.fields.version")}</span>
                                <Badge className="text-primary-foreground text-xs ms-2">
                              ...
                              <DropdownMenuSubContent>
                                {versions.map((v, index) => (
                                  <DropdownMenuItem
                                    key={`kebab-version-${index}`}
                                    className="flex items-center gap-2 cursor-pointer"
                                    onClick={() =>
                                      viewVersion(v.version.toString())
                                    }
                                  >
                                    ...
                                    {/* Signals that selecting a version navigates
                                        away to the version page. */}
                                    <LinkIcon className="ms-auto h-3.5 w-3.5 shrink-0 text-muted-foreground" />
                                  </DropdownMenuItem>
                                ))}
                              </DropdownMenuSubContent>
                            </DropdownMenuSub>
