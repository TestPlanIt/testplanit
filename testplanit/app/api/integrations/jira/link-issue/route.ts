...
const linkIssueSchema = z
  ...
  .refine(
    (data) =>
      data.testCaseId ||
      data.testRunId ||
      data.testRunResultId ||
      data.testRunStepResultId ||
      data.sessionId ||
      data.sessionResultId,
    {
      error:
        "One of testCaseId, testRunId, testRunResultId, testRunStepResultId, sessionId, or sessionResultId must be provided",
    }
  );
...
export async function POST(request: NextRequest) {
  try {
    ...
    let linkedIssues = [];
    if (linkData.testCaseId) {
      linkedIssues = await JiraLinkService.getLinkedJiraIssues(
        linkData.testCaseId
      );
    } else if (linkData.testRunId) {
      linkedIssues = await JiraLinkService.getLinkedJiraIssuesForTestRun(
        linkData.testRunId
      );
    } else if (linkData.testRunResultId) {
      linkedIssues = await JiraLinkService.getLinkedJiraIssuesForTestRunResult(
        linkData.testRunResultId
      );
    } else if (linkData.testRunStepResultId) {
      linkedIssues =
        await JiraLinkService.getLinkedJiraIssuesForTestRunStepResult(
          linkData.testRunStepResultId
        );
    } else if (linkData.sessionId) {
      linkedIssues = await JiraLinkService.getLinkedJiraIssuesForSession(
        linkData.sessionId
      );
    } else if (linkData.sessionResultId) {
      linkedIssues = await JiraLinkService.getLinkedJiraIssuesForSessionResult(
        linkData.sessionResultId
      );
    }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error ? error.message : "Failed to unlink issue",
      },
      { status: 500 }
    );
  }
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (params.testCaseId) {
      ...
      linkedIssues = await JiraLinkService.getLinkedJiraIssuesForTestRunResult(
        params.testRunResultId
      );
    } else if (params.testRunStepResultId) {
      linkedIssues =
        await JiraLinkService.getLinkedJiraIssuesForTestRunStepResult(
          params.testRunStepResultId
        );
