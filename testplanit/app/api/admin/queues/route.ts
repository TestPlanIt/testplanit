...
export async function GET(request: NextRequest) {
  try {
    ...
    const configuredConcurrency: Record<string, number> = {
      "forecast-updates": parseInt(
        process.env.FORECAST_CONCURRENCY ||
          String(defaultConcurrency["forecast-updates"]),
        10
      ),
      notifications: parseInt(
        process.env.NOTIFICATION_CONCURRENCY ||
          String(defaultConcurrency["notifications"]),
        10
      ),
      emails: parseInt(
        process.env.EMAIL_CONCURRENCY || String(defaultConcurrency["emails"]),
        10
      ),
      "issue-sync": parseInt(
        process.env.SYNC_CONCURRENCY ||
          String(defaultConcurrency["issue-sync"]),
        10
      ),
      "testmo-imports": parseInt(
        process.env.TESTMO_IMPORT_CONCURRENCY ||
          String(defaultConcurrency["testmo-imports"]),
        10
      ),
      "elasticsearch-reindex": parseInt(
        process.env.ELASTICSEARCH_REINDEX_CONCURRENCY ||
          String(defaultConcurrency["elasticsearch-reindex"]),
        10
      ),
      "audit-logs": parseInt(
        process.env.AUDIT_LOG_CONCURRENCY ||
          String(defaultConcurrency["audit-logs"]),
        10
      ),
      "budget-alerts": parseInt(
        process.env.BUDGET_ALERT_CONCURRENCY ||
          String(defaultConcurrency["budget-alerts"]),
        10
      ),
      "auto-tag": parseInt(
        process.env.AUTO_TAG_CONCURRENCY ||
          String(defaultConcurrency["auto-tag"]),
        10
      ),
      "repo-cache": parseInt(
        process.env.REPO_CACHE_CONCURRENCY ||
          String(defaultConcurrency["repo-cache"]),
        10
      ),
      "copy-move": parseInt(
        process.env.COPY_MOVE_CONCURRENCY ||
          String(defaultConcurrency["copy-move"]),
        10
      ),
      "duplicate-scan": parseInt(
        process.env.DUPLICATE_SCAN_CONCURRENCY ||
          String(defaultConcurrency["duplicate-scan"]),
        10
      ),
      "magic-select": parseInt(
        process.env.MAGIC_SELECT_CONCURRENCY ||
          String(defaultConcurrency["magic-select"]),
        10
      ),
      "step-scan": parseInt(
        process.env.STEP_SCAN_CONCURRENCY ||
    ...
    if (multiTenant && !currentTenantId) {
      return NextResponse.json(
        {
          error:
            "Multi-tenant mode is enabled but INSTANCE_TENANT_ID is not configured",
        },
        { status: 500 }
      );
    }
        ...
        try {
          ...
          if (multiTenant) {
            ...
            const filterByTenant = (jobs: any[]) =>
              jobs.filter(
                (job) => job != null && job.data?.tenantId === currentTenantId
              ).length;
