...
function createRequest(searchParams: Record<string, string>): NextRequest {
  const url = new URL("http://localhost/api/get-avatar-url");
...
describe("GET /api/get-avatar-url", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("returns signed URL on successful request", async () => {
    const request = createRequest({ prependString: "user123" });
  ...
  it("uses uploads/avatars/ key prefix", async () => {
    const request = createRequest({ prependString: "user456" });
    await GET(request);

    expect(mockPutObjectCommand).toHaveBeenCalledWith(
      expect.objectContaining({
        Key: expect.stringMatching(/^uploads\/avatars\//),
      })
    );
  });

  it("works without prependString (defaults to empty string)", async () => {
