...
describe("/api/report-builder/code-pin-coverage", () => {
  ...
  it("POST delegates with isCrossProject=false", async () => {
    (handleCodePinCoverageReportPOST as any).mockResolvedValue(
      Response.json({ data: [], total: 0 })
    );
    const req = new NextRequest(
      "http://localhost/api/report-builder/code-pin-coverage",
      {
        method: "POST",
    ...
    await POST(req);
    expect(handleCodePinCoverageReportPOST).toHaveBeenCalledWith(req, false);
  });
});
