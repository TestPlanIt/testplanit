...
const EmptyStateComponent = ({
  ...
  const tReports = (key: string) => {
    const translations: Record<string, string> = {
      ...
      selectAtLeastOneDimensionAndMetric:
        "Select at least one dimension and one metric.",
      noDataMatchingCriteria:
        "No data found matching the selected dimensions and metrics.",
      noDataMatchingCriteriaWithDateFilter:
        "No data found matching the selected dimensions and metrics. Try adjusting your date range filters.",
    };
...
describe("ReportBuilder Empty State", () => {
  it("should show 'select dimension and metric' message when no dimensions or metrics are selected", () => {
    render(
      <EmptyStateComponent lastUsedDimensions={[]} lastUsedMetrics={[]} />
    );

    expect(screen.getByText("No results found.")).toBeInTheDocument();
    expect(
      screen.getByText("Select at least one dimension and one metric.")
    ).toBeInTheDocument();
  });
  ...
  it("should not show date filter recommendation when date range is not set", () => {
    ...
    expect(
      screen.queryByText("Try adjusting your date range filters.")
