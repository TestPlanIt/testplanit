...
export async function seedDemoProject() {
  ...
  // Workflows
  const [caseDefaultWorkflow, caseActiveWorkflow, runDefaultWorkflow, sessionDefaultWorkflow] =
    await Promise.all([
      ...
      prisma.workflows.findFirst({ where: { scope: "SESSIONS", isDefault: true } }),
    ]);
  if (!caseDefaultWorkflow || !caseActiveWorkflow || !runDefaultWorkflow || !sessionDefaultWorkflow) {
    ...
    content: [
            ...
            content: [
              {
                ...
                attrs: { class: null, textAlign: "left" },
                content: [
                  { type: "text", marks: [{ type: "bold" }], text: "Repository" },
                  {
                    type: "text",
            ...
            content: [
              {
                ...
                attrs: { class: null, textAlign: "left" },
                content: [
                  { type: "text", marks: [{ type: "bold" }], text: "Test Runs" },
                  {
                    type: "text",
            ...
            content: [
              {
                ...
                attrs: { class: null, textAlign: "left" },
                content: [
                  { type: "text", marks: [{ type: "bold" }], text: "Milestones" },
                  {
                    type: "text",
      ...
      },
      {
        ...
        content: [
          {
            type: "text",
            text: 'When you\'re done exploring, you can remove this demo data by deleting the project from the project settings. All associated test cases, runs, sessions, and milestones will be cleaned up automatically.',
          },
        ],
      },
    ],
  ...
  const allStatuses = await prisma.status.findMany({ where: { isEnabled: true } });
  ...
  console.log("Created repository with folders: Root > Authentication, Dashboard");
    ...
    data: [
      {
        ...
        order: 1,
        step: JSON.stringify(tiptapText("Open the application URL in a supported browser")),
        expectedResult: JSON.stringify(tiptapText("Application home page loads successfully")),
      },
      {
        sharedStepGroupId: sharedStepGroup.id,
        order: 2,
        step: JSON.stringify(tiptapText("Verify the login page is accessible")),
        expectedResult: JSON.stringify(tiptapText("Login form is displayed with email and password fields")),
      },
      {
        sharedStepGroupId: sharedStepGroup.id,
        order: 3,
        step: JSON.stringify(tiptapText("Clear any cached sessions or cookies")),
        expectedResult: JSON.stringify(tiptapText("User is logged out and session is clean")),
      },
    ],
  });
  ...
  await createCaseFieldValues(case1.id, priorityField.id, descriptionField.id, priorityHigh?.id, "Verify that users can log in with valid email and password");
  await createSharedStepPlaceholder(case1.id, sharedStepGroup.id, 1);
  await createSteps(case1.id, [
    { step: "Enter valid email and password", expected: "Credentials are accepted" },
    { step: "Click the Sign In button", expected: "User is redirected to the dashboard" },
  ], 2);
  ...
  await createCaseFieldValues(case2.id, priorityField.id, descriptionField.id, priorityMedium?.id, "Verify new user registration flow with required fields");
  await createSharedStepPlaceholder(case2.id, sharedStepGroup.id, 1);
  await createSteps(case2.id, [
    { step: "Navigate to the registration page", expected: "Registration form is displayed" },
    { step: "Fill in name, email, and password", expected: "Form validates input in real time" },
    { step: "Submit the registration form", expected: "Account is created and confirmation message shown" },
  ], 2);
  ...
  await createCaseFieldValues(case3.id, priorityField.id, descriptionField.id, priorityMedium?.id, "Verify password reset via email link");
  await createSharedStepPlaceholder(case3.id, sharedStepGroup.id, 1);
  await createSteps(case3.id, [
    { step: "Click Forgot Password on the login page", expected: "Password reset form appears" },
    { step: "Enter registered email address", expected: "Success message displayed" },
    { step: "Open the reset link from email", expected: "New password form is shown" },
    { step: "Enter and confirm new password", expected: "Password is updated and user can log in" },
  ], 2);
  ...
  await createCaseFieldValues(case4.id, priorityField.id, descriptionField.id, priorityHigh?.id, "Verify all dashboard widgets render with correct data");
  await createSteps(case4.id, [
    { step: "Navigate to the project dashboard", expected: "Dashboard page loads without errors" },
    { step: "Verify all widgets display data", expected: "Charts, counters, and tables show correct information" },
  ]);
  ...
  await createCaseFieldValues(case5.id, priorityField.id, descriptionField.id, priorityLow?.id, "Verify dashboard data can be exported in supported formats");
  await createSteps(case5.id, [
    { step: "Navigate to the dashboard page", expected: "Dashboard is displayed" },
    { step: "Click the Export button", expected: "Export format options are shown" },
    { step: "Select CSV format and confirm", expected: "File downloads with correct data" },
  ]);
  ...
  const run1Statuses = [passedStatus, passedStatus, failedStatus, passedStatus, blockedStatus];
  ...
  console.log('Created Test Run 1: "Sprint 1 - Smoke Test" (3 passed, 1 failed, 1 blocked)');
  ...
  console.log('Created Test Run 2: "Sprint 2 - Regression" (2 passed, 1 retest, 1 untested)');
  ...
  const session1 = await prisma.sessions.create({
    data: {
      ...
      elapsed: 3600,
      mission: JSON.stringify(tiptapText(
      ...
      note: JSON.stringify(tiptapText(
  ...
  console.log('Created Session 1: "Exploratory - Login Flows" (2 passed, 1 failed)');
  ...
  console.log('Created Session 2: "Exploratory - Dashboard UX" (1 passed, 1 skipped)');
  ...
  console.log("  - 5 Repository Cases with steps, shared steps, and field values");
...
async function createSharedStepPlaceholder(
  ...
  await prisma.steps.create({
    data: {
      ...
      expectedResult: JSON.stringify({ type: "doc", content: [{ type: "paragraph" }] }),
    },
  });
}
