...
const MODELS = [
  ...modelsSrc.matchAll(/^export type (\w+) = \$ModelResult/gm),
].map((m) => m[1]);
  ...
  src = src.replace(HOOKS_IMPORT_RE, (full, typeOnly, body) => {
    ...
    for (const spec of specs) {
      ...
      if (parsed) {
        repl.set(
          local,
          `useClientQueries(schema).${parsed.accessor}.${parsed.method}`
        );
      } else {
...
if (unparsed.size) {
  ...
  for (const [n, c] of [...unparsed.entries()].sort((a, b) => b[1] - a[1]))
    console.log(`  ${n} (x${c})`);
}
if (DRY)
