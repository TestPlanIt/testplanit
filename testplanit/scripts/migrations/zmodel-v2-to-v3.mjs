...
s = s.replace(
  ...
  () => (log.push("removed generator client"), "")
);
...
// --- Rule fix 1: v3 cannot apply `!` to the nullable auth entity ---
{
  ...
  if (n) {
    s = s.replace(/!auth\(\)/g, "auth() == null");
    log.push(`!auth() -> auth() == null  x${n}`);
  }
}
...
// --- Rule fix 2: @password attribute removed in v3 (app hashes via bcrypt) ---
{
  ...
  if (n) {
    s = s.replace(/ @password\b/g, "");
    log.push(`removed @password  x${n}`);
  }
}
...
//     Notification: owner may only toggle isRead/isDeleted; admin unrestricted. ---
{
  ...
  if (s.includes(v2)) {
    s = s.replace(v2, v3);
    log.push("rewrote future() -> post-update (Notification)");
  }
}
...
for (const frag of [
  ...
  if (s.includes(frag)) {
    s = s.replace(
      frag,
      frag.replace("@@allow('all',", "@@allow('read,update,delete',")
    );
...
{
  const re =
    /(@(?:allow|deny)\('[^']*',[^,()]*(?:==|!=|>|<|>=|<=)[^,()]*),\s*(?:true|false)\)/g;
  const n = (s.match(re) || []).length;
  if (n) {
    s = s.replace(re, "$1)");
    log.push(`dropped field-level policy override arg  x${n}`);
  }
}
