...
const generateLargeCSV = () => {
  ...
  for (let i = 1; i <= 5000; i++) {
    ...
    const automated =
      automationStatus[Math.floor(Math.random() * automationStatus.length)];
  ...
  // Write to file
  const outputPath =
    "/Users/bdermanouelian/git/testplanit/testplanit/test/large_import_test_cases.csv";
  writeFileSync(outputPath, csvContent);

  console.log(
    `✅ Generated large CSV file with 5,000 test cases at: ${outputPath}`
  );
  console.log(
