...
class TestDataGenerator {
  ...
  async generateData(
  ...
  ): Promise<{ folders: number; cases: number; timeMs: number }> {
    const startTime = performance.now();
    console.log(`\n📊 Generating ${config.name}...`);
    console.log(
      `Target: ${config.folders} folders, depth: ${config.maxDepth}, cases: ${config.casesPerFolder} per folder`
    );
    ...
    for (let i = 0; i < folders.length; i += folderBatchSize) {
      ...
      if (i % 5000 === 0) {
        console.log(
          `  📁 Created ${Math.min(i + folderBatchSize, folders.length)}/${folders.length} folders`
        );
      }
    }
    ...
    if (config.casesPerFolder > 0) {
      ...
      for (let i = 0; i < cases.length; i += caseBatchSize) {
        ...
        if (i % 10000 === 0) {
          console.log(
            `  📋 Created ${Math.min(i + caseBatchSize, cases.length)}/${cases.length} test cases`
          );
        }
      }
    }
    ...
    console.log(
      `✅ Generated ${folders.length} folders and ${cases.length} test cases in ${(timeMs / 1000).toFixed(2)}s`
    );
  ...
  async benchmarkTreeViewPerformance(config: TestDataConfig): Promise<{
    ...
    if (totalTime < targetTime) {
      console.log(
        `    ✅ EXCELLENT performance (${((targetTime / totalTime) * 100).toFixed(0)}% faster than target)`
      );
    } else if (totalTime < targetTime * 1.5) {
      console.log(
        `    ⚠️  ACCEPTABLE performance (${((totalTime / targetTime) * 100 - 100).toFixed(0)}% slower than target)`
      );
    } else {
      console.log(
        `    ❌ POOR performance (${((totalTime / targetTime) * 100 - 100).toFixed(0)}% slower than target)`
      );
    }
  ...
  async runFullBenchmark(): Promise<void> {
    ...
    results.forEach(({ config, generation, performance }, index) => {
      ...
      console.log(
        `   Generation Time: ${(generation.timeMs / 1000).toFixed(2)}s`
      );
      ...
      console.log(
        `   Total TreeView Time: ${(performance.fetchTime + performance.renderTime).toFixed(2)}ms`
      );
    });
