...
// Coalescing-aware tx mock. scim.group.member_added routes through
// emitWithCoalescing which calls tx.webhookConfig.findMany + count + create
// on webhookEventDedup. Under threshold the helper falls through to
// webhookEvents.emit with the original per-event payload, preserving the
// existing emit assertions.
