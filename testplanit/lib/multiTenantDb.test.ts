...
describe("multiTenantDb", () => {
  ...
  describe("config file caching", () => {
    ...
    it("getTenantDbClient force-reloads once for a tenant missing from the cache", async () => {
      fs.writeFileSync(
        configFile,
        JSON.stringify({
          "tenant-a": { databaseUrl: "postgresql://localhost/a" },
      ...
      const { loadTenantConfigs, getTenantDbClient } = await resetModule();
      ...
      expect(getTenantDbClient("tenant-b")).toBeTruthy();

      // A genuinely unknown tenant still throws after the forced reload.
      expect(() => getTenantDbClient("tenant-nope")).toThrow(
