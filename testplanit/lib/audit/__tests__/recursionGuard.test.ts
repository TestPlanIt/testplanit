...
describeDb(
  "recursionGuard — no cascade (SAF-03) + append-only enforcement (success criterion #5)",
  () => {
    ...
    it("(i) a single mutation produces EXACTLY ONE DataChangeLog row, not N (SAF-03 recursion guard)", async () => {
      ...
      await direct.query("SELECT set_config('app.audit_context', $1, true)", [
        ctx({ userId: "recguard-actor" }),
      ]);
      await direct.query(
        `UPDATE "RepositoryCases" SET name = $1 WHERE id = $2`,
        [`${MARKER}-updated`, seededCaseId]
      );
    ...
    it('(ii.1) UPDATE "DataChangeLog" SET actor=... RAISES a privilege error (42501) — append-only', async () => {
      await expect(
        direct.query(
          `UPDATE "DataChangeLog" SET actor = 'tamper' WHERE id = $1`,
          [fixtureLogId]
        )
    ...
    it('(ii.2) DELETE FROM "DataChangeLog" RAISES a privilege error (42501) — append-only', async () => {
      await expect(
        direct.query(`DELETE FROM "DataChangeLog" WHERE id = $1`, [
          fixtureLogId,
        ])
      ).rejects.toMatchObject({ code: PRIVILEGE_ERROR });
    });
    ...
    it('(ii.3) worker-cursor UPDATE "DataChangeLog" SET processed=true SUCCEEDS — worker path stays open', async () => {
      ...
      await expect(
        direct.query(
          `UPDATE "DataChangeLog" SET processed = true WHERE id = $1`,
          [fixtureLogId]
        )
      ).resolves.toBeDefined();

      const r = await direct.query(
        `SELECT processed FROM "DataChangeLog" WHERE id = $1`,
        [fixtureLogId]
      );
