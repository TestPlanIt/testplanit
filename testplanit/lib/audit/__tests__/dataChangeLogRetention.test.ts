...
describeDb(
  "dataChangeLogRetention — processed=true rows pruned, unprocessed rows never deleted",
  () => {
    ...
    it("seeds rows and runs purgeOnce, then asserts old-processed deleted, recent-processed and unprocessed survive", async () => {
      ...
      const oldProcessedResult = await direct.query(
       ...
       RETURNING id`,
        [
          MARKER,
          JSON.stringify({ col: { old: "a", new: "b" } }),
          `${MARKER}-actor`,
        ]
      );
      const oldProcessedIds: bigint[] = oldProcessedResult.rows.map(
        (r: { id: bigint }) => BigInt(r.id)
      );
      ...
      const recentProcessedResult = await direct.query(
       ...
       RETURNING id`,
        [
          MARKER,
          JSON.stringify({ col: { old: "c", new: "d" } }),
          `${MARKER}-actor`,
        ]
      );
      const recentProcessedIds: bigint[] = recentProcessedResult.rows.map(
        (r: { id: bigint }) => BigInt(r.id)
      );
      ...
      const unprocessedResult = await direct.query(
       ...
       RETURNING id`,
        [
          MARKER,
          JSON.stringify({ col: { old: null, new: "e" } }),
          `${MARKER}-actor`,
        ]
      );
      const unprocessedRowIds: bigint[] = unprocessedResult.rows.map(
        (r: { id: bigint }) => BigInt(r.id)
      );
    ...
    it("tamper-evidence: direct DELETE of an unprocessed row is rejected with 42501 (datachangelog_append_only carve-out)", async () => {
      ...
      if (unprocessedIds.length > 0) {
        ...
        const r = await direct.query(
         ...
         RETURNING id`,
          [
            MARKER,
            JSON.stringify({ col: { old: null, new: "f" } }),
            `${MARKER}-actor`,
          ]
        );
      ...
      try {
        await direct.query(`DELETE FROM "DataChangeLog" WHERE id = $1`, [
          String(targetId),
        ]);
      } catch (err) {
        caughtError = err;
      ...
      const errMsg =
        (caughtError as { message?: string; code?: string })?.message ?? "";
      const errCode = (caughtError as { code?: string })?.code ?? "";
      expect(
        errCode === "42501" ||
          errMsg.toLowerCase().includes("not permitted") ||
          errMsg.toLowerCase().includes("append-only")
      ).toBe(true);
    });
  }
);
