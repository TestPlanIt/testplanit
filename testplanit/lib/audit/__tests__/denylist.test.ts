...
describeDb("denylist — TipTap columns omitted + 32KB size guard (SAF-02 / success criterion #4)", () => {
    ...
    if (direct) {
      try {
        if (createdStepIds.length) {
          await direct.query(`DELETE FROM "Steps" WHERE id = ANY($1)`, [createdStepIds]);
        }
  ...
  it("a Steps INSERT with TipTap step/expectedResult → changed_cols OMITS step and expectedResult (SAF-02)", async () => {
    const tiptap = JSON.stringify({ type: "doc", content: [{ type: "paragraph", content: [{ type: "text", text: `${MARKER}-tiptap` }] }] });
    const order = Math.floor(Math.random() * 1_000_000);

    await direct.query("BEGIN");
    await direct.query("SELECT set_config('app.audit_context', $1, true)", [ctx({ userId: "denylist-actor" })]);
  ...
  it("a >32KB value in a captured column is omitted by the size guard", async () => {
    ...
    await direct.query("BEGIN");
    await direct.query("SELECT set_config('app.audit_context', $1, true)", [ctx({ userId: "denylist-actor" })]);
    await direct.query(`UPDATE "RepositoryCases" SET "className" = $1, name = $2 WHERE id = $3`, [
      huge,
      smallName,
      seededCaseId,
    ]);
