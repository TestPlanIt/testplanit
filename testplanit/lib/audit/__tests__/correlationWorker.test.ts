...
describeDb("correlationWorker (COR-01 + CTX-03) — DataChangeLog → AuditLog materialization", () => {
  ...
  const seedDcl = async (op: string, pk: string, operationId: string | null, name: string) => {
    await direct.query(
  ...
  const countAuditLogsForOp = async (operationId: string): Promise<number> => {
  ...
  const auditRowsForOp = async (operationId: string) => {
    ...
    return r.rows as Array<{
      ...
      entityId: string;
      action: string;
      changes: Record<string, { old?: unknown; new?: unknown; oldName?: unknown; newName?: unknown }>;
      sourceTable: string;
      operationId: string;
    }>;
  };
  ...
  beforeAll(async () => {
    ...
    ({ pollDataChangeLogsOnce } = await import(/* @vite-ignore */ correlationMod));
  ...
  afterAll(async () => {
    if (direct) {
      try {
        await direct.query(`DELETE FROM "AuditLog" WHERE "operationId" = $1`, [OP_ID]);
        await direct.query(`DELETE FROM "AuditLog" WHERE "operationId" = $1`, [`${OP_ID}-rollup`]);
        if (seededFieldId != null) {
          await direct.query(`DELETE FROM "CaseFields" WHERE id = $1`, [seededFieldId]);
        }
        ...
        await direct.query(`DELETE FROM "DataChangeLog" WHERE operation_id = $1`, [OP_ID]);
      } catch {
  ...
  it("COR-01 + CTX-03: polls unprocessed rows, materializes AuditLog stamped with the shared operationId, marks processed=true", async () => {
    ...
    const result = await pollDataChangeLogsOnce(prismaBase, { batchSize: 500 });
  ...
  it("HEADLINE UAT (criteria 1+2+3): a 3-request case save sharing ONE operationId → one grouped operationId, children rolled up to the parent RepositoryCases, and field ids humanized to names", async () => {
    ...
    expect(new Set(rows.map((r) => r.operationId))).toEqual(new Set([ROLLUP_OP]));
  ...
  it("crash-resume: a batch transaction that aborts mid-flush rolls back both the inserts AND the cursor, and a clean re-poll yields exactly ONE set of AuditLog rows (no duplicates)", async () => {
        ...
        if (prop === "$transaction") {
          return (fn: (tx: unknown) => Promise<unknown>) =>
            target.$transaction(async (tx: unknown) => {
              ...
              throw new SimulatedCrash("simulated mid-batch crash before commit");
            });
        }
    ...
    await direct.query(`DELETE FROM "AuditLog" WHERE "operationId" = $1`, [RESUME_OP]);
    await direct.query(
  ...
  it("defense-in-depth: even if AuditLog commits WITHOUT the cursor advancing, the ON CONFLICT idempotency index makes the re-poll add ZERO duplicate rows", async () => {
    ...
    try {
      ...
      await pollDataChangeLogsOnce(prismaBase, { batchSize: 500, markProcessed: false });
