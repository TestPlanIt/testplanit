...
describeDb("credentialDenylist — Integration.credentials never reaches DataChangeLog (SAF-02 / COV-04)", () => {
    ...
    if (direct) {
      try {
        if (createdIntegrationIds.length) {
          await direct.query(`DELETE FROM "Integration" WHERE id = ANY($1)`, [createdIntegrationIds]);
        }
      } catch {
        /* best effort */
      }
  ...
  it("INSERT into Integration with credentials → DataChangeLog row EXISTS, changed_cols OMITS credentials (SAF-02)", async () => {
    const creds = JSON.stringify({ api_key: `${MARKER}-secret-key`, token: "should-never-appear" });
    const name = `${MARKER}-integration`;

    await direct.query("BEGIN");
    await direct.query("SELECT set_config('app.audit_context', $1, true)", [ctx({ userId: "credtest-actor" })]);
  ...
  it("UPDATE of Integration changing credentials alongside name → changed_cols OMITS credentials, INCLUDES name (SAF-02)", async () => {
    ...
    const credsOriginal = JSON.stringify({ api_key: `${MARKER}-original-secret` });

    await direct.query("BEGIN");
    await direct.query("SELECT set_config('app.audit_context', $1, true)", [ctx({ userId: "credtest-actor-insert" })]);
    const ins = await direct.query(
      `INSERT INTO "Integration" (name, provider, "authType", credentials, status, "updatedAt")
    ...
    const credsUpdated = JSON.stringify({ api_key: `${MARKER}-new-secret`, token: "still-should-not-appear" });

    await direct.query("BEGIN");
    await direct.query("SELECT set_config('app.audit_context', $1, true)", [ctx({ userId: "credtest-actor-update" })]);
    await direct.query(
