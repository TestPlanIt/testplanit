 ...
 * Slack formatter. Single source of truth so the component and the webhook
 * never disagree on what "Passed/Failed/Pending" mean.
 *
 ...
 *
 * Returns counts and a `completionPct` derived from completed/total.
 * `passed + failed` may be less than `completed` (skipped cases land in
 * "completed but neither success nor failure").
 */
...
/**
 * D-15 — TestRunSummaryData payload shape used by:
 *  - GET /api/test-runs/[testRunId]/summary (in-app summary UI)
 *  - test_run.completed outbound webhook payload (via emitTestRunUpdateEvents)
 *
 * Lifted verbatim from app/api/test-runs/[testRunId]/summary/route.ts (Plan
 * 02-05 Task 5.1 refactor) so the route handler and the webhook emitter
 * share the same data assembly logic.
 */
