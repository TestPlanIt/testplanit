 ...
 *
 *   2. If the target Workflows row has `requiresReview === false` (or does
 ...
 *
 *   3. If `requiresReview === true`, the helper looks for an approved AND
 ...
 *
 *   4. On a hit, the helper returns `{ approvedRequestId }` so the caller
 ...
 *
 *   5. On a miss, the helper throws `ReviewGateError` with
 *      `code === 'REVIEW_REQUIRED'` and the full
 *      `(entityType, entityId, toStateId)` payload so chokepoint routes can
 *      translate it to a structured 403 response (Plan 04 wires that path).
 *
 ...
 * (Phase 2 work) needs raw-Prisma semantics to bypass the append-only
 ...
 *                    polymorphically (no FK back-relation, per D-14).
 ...
 *
 * @returns           `null` when the target state does not require review
 *                    (no gate applied); `{ approvedRequestId }` when an
 *                    approved + unconsumed ReviewRequest exists for the
 *                    transition.
 *
