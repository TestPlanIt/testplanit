...
describe("matchTagSuggestions", () => {
  it("matches exact case-insensitive tags as existing", () => {
    const result = matchTagSuggestions(["Login"], ["login", "regression"], []);
  ...
  it("marks unmatched tags as new", () => {
    const result = matchTagSuggestions(["api"], ["login", "regression"], []);
  ...
  it("returns all as new when existingProjectTags is empty", () => {
    const result = matchTagSuggestions(["login", "api", "smoke"], [], []);
    expect(result).toHaveLength(3);
  ...
  it("fuzzy matches close variants via Levenshtein distance", () => {
    ...
    const result = matchTagSuggestions(["ui test"], ["ui testing"], []);
    expect(result).toEqual([
      {
        tagName: "ui test",
        isExisting: true,
  ...
  it("trims whitespace from AI suggestions", () => {
    const result = matchTagSuggestions(["  login  "], ["login"], []);
    expect(result).toHaveLength(1);
