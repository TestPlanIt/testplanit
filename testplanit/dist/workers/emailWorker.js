      ...
      (res) => {
        ...
        res.on("end", () => {
          ...
          if (!res.statusCode || res.statusCode >= 400) {
            reject(
              new Error(
                `Kubernetes API ${res.statusCode} for ${path3}: ${body.slice(0, 200)}`
              )
            );
            return;
          }
...
async function fetchEncryptionKey(tenantId) {
  ...
  const path3 = `/api/v1/namespaces/${encodeURIComponent(namespace)}/secrets/${encodeURIComponent(secretName)}`;
  const secret = await kubeGet(path3);
...
var startWorker = async () => {
  ...
  if (valkey_default) {
    worker = new import_bullmq2.Worker(EMAIL_QUEUE_NAME, withTenantContext(processor), {
