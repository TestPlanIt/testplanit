...
var processor2 = async (job) => {
  ...
  switch (job.name) {
    case JOB_CREATE_NOTIFICATION:
      ...
      try {
        const userPreferences = await prisma9.userPreferences.findUnique({
          where: { userId: createData.userId }
        });
        const globalSettings = await prisma9.appConfig.findUnique({
        ...
        const notification = await prisma9.notification.create({
          data: {
    ...
    case JOB_PROCESS_USER_NOTIFICATIONS:
      ...
      try {
        const notifications = await prisma9.notification.findMany({
          where: {
    ...
    case JOB_SEND_DAILY_DIGEST:
      try {
        ...
        const users = await prisma9.userPreferences.findMany({
          where: {
            OR: [
