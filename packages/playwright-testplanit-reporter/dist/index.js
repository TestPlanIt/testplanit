...
function guessMimeType(fileName) {
  return MIME_TYPES[path__namespace.extname(fileName).toLowerCase()] ?? "application/octet-stream";
}
...
async function attachToRun(testInfo, input) {
  ...
  if (file.path) {
    const name = file.name && file.name.trim() ? file.name : path__namespace.basename(file.path);
...
${error.stack}` : "";
  ...
  async uploadAttachments(result, attachments) {
    ...
    const uploads = filtered.map(async (att) => {
      try {
        const buffer = att.body ?? (att.path ? await fs__namespace.readFile(att.path) : void 0);
        if (!buffer || buffer.length === 0) {
          this.log(`Skipping empty attachment "${att.name}" for ${result.testName}`);
          return;
        }
        const baseName = att.path ? path__namespace.basename(att.path) : `${att.name || "attachment"}${this.extForContentType(att.contentType)}`;
  ...
  async applyRunLevelOp(op) {
    try {
      ...
      switch (op.kind) {
        ...
        case "file": {
          const buffer = op.body ?? (op.path ? await fs__namespace.readFile(op.path) : void 0);
          if (!buffer || buffer.length === 0) {
  ...
  async uploadRunFile(testRunId, input) {
    ...
    if (input.buffer) {
      ...
      try {
        buffer = await fs__namespace.readFile(input.path);
      } catch {
        return false;
      }
      name = input.name && input.name.trim() ? input.name : path__namespace.basename(input.path);
    } else {
  ...
  formatRunName(template) {
    ...
    return template.replace("{date}", date).replace("{time}", time).replace("{browser}", browser).replace("{platform}", platform).replace("{spec}", spec).replace("{shard}", shard).replace("{suite}", suite);
  }
...
exports.attachToRun = attachToRun;
exports.default = TestPlanItReporter;
exports.setRunMetadata = setRunMetadata;
