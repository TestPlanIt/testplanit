...
**Back up your database before upgrading.** 1.0 moves schema changes to versioned migrations, and a database created by a 0.x release needs a one-time baseline step before its first v1.0 start — see the [upgrade notes](/docs/installation#upgrading) for the exact commands.
