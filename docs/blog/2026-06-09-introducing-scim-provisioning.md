...
4. (Optional) Subscribe a Slack channel or a custom HMAC endpoint to the SCIM lifecycle events at **Admin → Tools & Integrations → System Webhooks** so provisioning activity mirrors into your existing ops channels. Same machinery as the [project-level webhooks](/blog/introducing-webhooks), just scoped to system events.
