...
TestPlanIt does something different. When you run your suite through the [WebdriverIO reporter](/docs/sdk/wdio-test-cases), it **captures the actual sequence of commands each test executed** — every navigation, element lookup, click, keystroke, and assertion — and hands that command stream to **your own LLM**, which turns it into readable, user-facing steps. The result describes what the test *actually did*, not a guess from its title. (When the commands aren't available — a plain results import only carries the test's name — it infers the steps from that descriptive name instead.)
