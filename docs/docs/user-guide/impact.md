...
- **Why** — one reason badge per signal that selected the case: **Pin**, **Issue**, **Keyword**, **History**, **AI**, or **Related**. Hover a badge for its detail; **Show reasons** expands the row with the full list, including the AI's rationale.
...
| The search index holds no test cases for this project…                                                             | Elasticsearch answered, but nothing from this project has been indexed, so keyword matching fell back to test case names in the database and matches on case content may be missed. An administrator rebuilds the index under **Administration → Search Engine**; new and edited cases are indexed as they change, but cases that existed before the index was set up need that one rebuild. |
