...
The **Close abandoned automated runs automatically** switch turns the policy on and off:

- **Off**: No system-wide cleanup. Individual projects can still opt in through their [Advanced settings](./projects.md#abandoned-automation-cleanup). This is the default, so existing installations are unaffected on upgrade.
- **On**: Set the idle threshold in minutes (minimum 15, matching the sweep cadence). A run is considered abandoned only when _nothing_ has been imported for that long — an in-flight run with a quiet stretch is left alone, so pick a threshold comfortably longer than the longest pause a healthy run can have. A full day (1440 minutes) is a safe default.

After setting the switch (and a threshold, where applicable), click **Save policy**.
