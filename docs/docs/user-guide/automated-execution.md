...
3. TestPlanIt records an **execution** for the run, promotes a manual run to a [hybrid run](projects/run-details.md#hybrid-test-runs), writes the chosen parameter values into the run's documentation as `Label: value` lines (the same run metadata the reporters write, so a run says what it was executed with even after the job is gone), and starts the job with these parameters:
