...
- **Parameters** — values the person executing a run chooses in the **Execute automated cases** dialog, so one target and one CI job can serve several browsers, environments or devices. Each parameter has an **Input name** (the key the job receives, same rules as a static input), a **Label** shown in the dialog, and a **Type**:
  ...
  - **Configuration** — one of the [configurations](../../configurations.md) assigned to the project (or several, with **Allow choosing more than one**), with an optional default. The dialog lists the project's enabled configurations by name. The job receives the choice as three inputs, so it can act on whichever it prefers: `<name>_ID` (the configuration id, `12` or `12,15`), `<name>` (the configuration name, comma-separated when several) and `<name>_VARIANTS` (each configuration's variants as `Category=Variant` pairs, comma-separated, with several configurations separated by `;`, for example `Browser=Chrome,OS=Windows 11;Browser=Safari,OS=macOS`). A configuration that is later unassigned or disabled is no longer offered, and an execution naming it is refused.

  Every parameter is sent on every dispatch, with its default when the dispatcher leaves it alone. An input name cannot start with `TESTPLANIT_`, cannot repeat within the target, and cannot match one of the target's static inputs; a configuration parameter also reserves its `_ID` and `_VARIANTS` names. Static inputs and parameters together are limited to 20 inputs, a configuration parameter counting as three. On GitHub each parameter (all three inputs of a configuration parameter) must also be declared under `workflow_dispatch.inputs`.
