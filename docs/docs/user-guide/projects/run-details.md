...
- **Action Buttons**:
  - **View Mode (Active Run)**:
    ...
    - **Execute automated cases** (in the case list header, left of **Start manual testing**): Dispatches the run's automated cases to one of the project's execution targets. Select rows in the case list to execute only those: the button then reads **Execute N selected automated cases** and requests just the automated cases among the selection. Shown when the project has an enabled target and the user can add/edit runs; disabled while the run has no automated cases, none of the selected cases are automated, or an execution is already in flight. See [Automated execution](#automated-execution).
...
When the project has an [execution target](settings/automation.md), the case list header offers **Execute automated cases** next to **Start manual testing**. The dialog picks the target, optionally a branch or ref, and states how many automated cases will be requested. To execute a subset, select rows in the case list first: the button reads **Execute N selected automated cases**, the request names only the automated cases among the selection, and the dialog says how many selected cases are skipped because they are not automated. **Retry** on an execution replays the same subset. TestPlanIt then starts the job with the run id; the job reads the plan of cases, runs them, and reports results into this run — see [Automated Execution](../automated-execution.md) for the job's side.

Once a run has been executed, an execution chip appears under the header actions with the latest execution's status, the parameter values it was requested with (if the target declares any), a link to the job on the provider, who requested it and when, and the provider's own status when it can be read. While an execution is in flight the chip offers **Cancel**; after a failure it offers **Retry**. The history icon opens a sheet listing every execution of the run, newest first, each with its parameters.
