...
2. A modal dialog will appear. Fill in the user details:
   ...
   - **Access**: Select the system access level (ADMIN, PROJECTADMIN, USER, NONE). Defaults to USER. If the user belongs to a group with a [Mapped Access Tier](./groups.md#mapped-access-tier), or your identity provider asserts a mapped [role](./scim.md#map-an-idp-role-to-an-access-tier), their access is instead governed by directory mapping (a mapped role wins outright; otherwise the highest group tier wins, with a configurable fallback default).
...
3. A modal dialog will appear. You can modify:
   ...
   - Access Level (Cannot change your own access level). If this user's access is governed by group or role mapping, the dialog shows a **Group Mapped** badge and a **Managed by Group Mapping** warning; saving a manual change switches them to manual control.
