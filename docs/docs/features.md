...
- **Per-project group access** - Grant a group an access tier on [specific projects only](./user-guide/groups.md#per-project-access) instead of org-wide, resolved through the same permission rules as any other project grant, so membership changes from your IdP carry per-project access with no further configuration.
