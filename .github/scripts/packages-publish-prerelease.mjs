// Publish pre-release packages from a release branch under the `beta` npm tag.
//
// A release branch (today `release/v1.1`, a semantic-release prerelease branch
// for the app) carries package changes that depend on the unreleased app line
// and therefore cannot ship to `latest` — a user on the released app would
// install them and hit missing endpoints. They go out under the `beta`
// dist-tag instead, so `npm i @testplanit/mcp-server` keeps resolving to the
// `latest` release and testers opt in explicitly with `@beta`.
//
// Pre-releases are NOT Changesets-managed. Changesets versions from `main`
// only (its "Version Packages" PR never lands on a release branch, which is
// why most packages/* versions here trail or equal npm). Cutting a pre-release
// is instead a deliberate manual step: set the package's version to an
// explicit pre-release on the release branch and push. Versions track the
// app's line — `1.1.0-beta.N` for release/v1.1 — which is ahead of every
// packages/* `latest`.
//
// That drives the safety gate below: this script publishes a package ONLY when
// its version contains a pre-release identifier. Every other packages/* on the
// branch carries a plain version that is already on npm (or trails it), so
// without the gate a push would try to republish stale `latest` versions.
//
...
//
// PACKAGES_PUBLISH_DRY_RUN=1 runs `pnpm publish --dry-run` (packs, publishes
// nothing) so the gate and the pack can be checked locally.
import { readFileSync, readdirSync, existsSync, appendFileSync } from "node:fs";
import { spawnSync } from "node:child_process";

const TAG = "beta";
const DRY_RUN = process.env.PACKAGES_PUBLISH_DRY_RUN === "1";
...
for (const { dir, pkg } of candidates) {
  ...
  console.log(
    `[packages-publish-prerelease] publishing ${pkg.name}@${pkg.version} --tag ${TAG}`
  );
  ...
  if (result.status === 0) {
    ...
    console.error(
      `[packages-publish-prerelease] FAILED to publish ${pkg.name}@${pkg.version}`
    );
  }
}

const summary = [
  DRY_RUN ? "## Beta Pre-releases (dry run)" : "## Beta Pre-releases",
  "",
  published.length
