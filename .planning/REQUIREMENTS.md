...
- [ ] **WARN-01**: User sees a soft, non-blocking warning after creating a test case that resembles an existing one
...
- [ ] **RES-02**: User can merge two cases — surviving case inherits all test run history, steps, attachments, tags, and field values
- [ ] **RES-03**: Merge handles TestRunCases unique constraint conflicts when both cases appear in the same test run
- [ ] **RES-04**: Merge preserves version history from the merged case (re-parents versions to survivor before soft-deleting)
- [ ] **RES-05**: Merge executes as a single atomic transaction — no partial merge state is observable
- [ ] **RES-06**: User can link two cases as related using RepositoryCaseLink (SAME_TEST_DIFFERENT_SOURCE) without merging
- [ ] **RES-07**: User can dismiss a candidate pair as not-duplicate so it does not resurface in future scans
...
| RES-04      | Phase 49 | Pending |
| RES-05      | Phase 49 | Pending |
| RES-06      | Phase 49 | Pending |
| RES-07      | Phase 49 | Pending |
